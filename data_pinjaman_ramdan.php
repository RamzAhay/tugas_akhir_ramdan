...
$filter_anggota = isset($_GET['id_anggota_ramdan']) ? mysqli_real_escape_string($koneksi, $_GET['id_anggota_ramdan']) : '';
...
<div class="content">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <?php if ($role_user != 'Anggota'): ?>
            <a href="tambah_pinjaman_ramdan.php" class="btn btn-dark btn-sm px-3 shadow-sm" style="background: var(--ksp-theme); border: none;">+ Pinjaman Baru</a>
        <?php endif; ?>
    </div>

    <!-- AREA FILTER -->
    <div class="filter-section shadow-sm">
        <form method="GET" action="data_pinjaman_ramdan.php">
            <div class="filter-flex-container">
                
                <?php if($role_user != 'Anggota'): ?>
                <div class="filter-item">
                    <span class="small fw-bold text-muted text-uppercase mb-1 d-block">Anggota</span>
                    <select name="id_anggota_ramdan" class="f-input-small">
                        <option value="">-- Semua --</option>
                        <?php
                        $q_opt = mysqli_query($koneksi, "SELECT * FROM tb_anggota_ramdan ORDER BY nama_ramdan ASC");
                        while($d_opt = mysqli_fetch_assoc($q_opt)) {
                            $sel = ($filter_anggota == $d_opt['id_anggota_ramdan']) ? 'selected' : '';
                            echo "<option value='".$d_opt['id_anggota_ramdan']."' $sel>".$d_opt['nama_ramdan']."</option>";
                        }
                        ?>
                    </select>
                </div>
                ...
                <div class="d-flex gap-1">
                    ...
                    $params_cetak = http_build_query([
                        'id_anggota_ramdan' => $filter_anggota,
                        'status' => $filter_status,
                        'tgl_awal' => $filter_tgl_awal,
                        'tgl_akhir' => $filter_tgl_akhir
                    ]);
                    ?>
                    <a href="cetak_laporan_pinjaman_ramdan.php?<?php echo $params_cetak; ?>" 
                       class="btn-ksp" target="_blank">
                        <i class="bi bi-printer me-2"></i> CETAK
                    </a>

                    <?php if($filter_anggota != '' || $filter_status != '' || $filter_tgl_awal != '' || $filter_tgl_akhir != ''): ?>
                        <a href="data_pinjaman_ramdan.php" class="btn-f-reset" title="Bersihkan Filter">
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        <?php
                        $no = 1;
                        $sql = "SELECT p.*, a.nama_ramdan 
                                FROM tb_pinjaman_ramdan p 
                                JOIN tb_anggota_ramdan a ON p.id_anggota_ramdan = a.id_anggota_ramdan 
                                WHERE p.status_pinjaman_ramdan NOT IN ('Lunas', 'Ditolak')";

                        if ($filter_anggota != '') $sql .= " AND p.id_anggota_ramdan = '$filter_anggota'";
                        if ($filter_status != '') $sql .= " AND p.status_pinjaman_ramdan = '$filter_status'";
                        
                        if ($filter_tgl_awal != '' && $filter_tgl_akhir != '') {
                            $sql .= " AND p.tanggal_pinjaman_ramdan BETWEEN '$filter_tgl_awal' AND '$filter_tgl_akhir'";
                        } elseif ($filter_tgl_awal != '') {
                            $sql .= " AND p.tanggal_pinjaman_ramdan >= '$filter_tgl_awal'";
                        } elseif ($filter_tgl_akhir != '') {
                            $sql .= " AND p.tanggal_pinjaman_ramdan <= '$filter_tgl_akhir'";
                        }

                        $sql .= " ORDER BY p.id_pinjaman_ramdan DESC";
                        ...
                        while ($data = mysqli_fetch_assoc($query)) {
                            $status = $data['status_pinjaman_ramdan'];
                            $badge_class = ($status == 'Diajukan') ? 'badge-pending' : 'badge-active';
                        ?>
                        <tr>
                            <td class="text-center text-muted small"><?php echo $no++; ?></td>
                            <td class="fw-bold text-dark"><?php echo htmlspecialchars($data['nama_ramdan']); ?></td>
                            <td class="text-center small"><?php echo date('d/m/Y', strtotime($data['tanggal_pinjaman_ramdan'])); ?></td>
                            <td class="small"><?php echo rupiah($data['jumlah_pinjaman_ramdan']); ?></td>
                            <td class="fw-bold text-dark"><?php echo rupiah($data['sisa_pinjaman_ramdan']); ?></td>
                            ...
                            <td class="text-center">
                                <div class="d-flex justify-content-center gap-1">
                                    <a href="riwayat_angsuran_ramdan.php?id=<?php echo $data['id_pinjaman_ramdan']; ?>" class="btn-ksp-outline" title="Lihat Riwayat">Detail</a>
                                    
                                    <?php if ($data['status_pinjaman_ramdan'] == 'Diajukan'): ?>
                                        
                                        <!-- CRITICAL BUG FIX: Tombol ACC & Tolak hanya untuk Admin -->
                                        <?php if ($role_user == 'Admin'): ?>
                                            <a href="acc_pinjaman_ramdan.php?id=<?php echo $data['id_pinjaman_ramdan']; ?>" class="btn-ksp-outline" style="border-color: #10b981; color: #10b981;" onclick="return confirm('Setujui pinjaman ini?')">ACC</a>
                                            <a href="tolak_pinjaman_ramdan.php?id=<?php echo $data['id_pinjaman_ramdan']; ?>" class="btn-ksp-outline" style="border-color: #ef4444; color: #ef4444;" onclick="return confirm('Tolak pinjaman ini?')">Tolak</a>
                                        <?php endif; ?>

                                        <!-- Petugas & Admin masih boleh edit sebelum di-ACC -->
                                        <?php if ($role_user == 'Admin' || $role_user == 'Petugas'): ?>
                                            <a href="edit_pinjaman_ramdan.php?id=<?php echo $data['id_pinjaman_ramdan']; ?>" class="btn-ksp-outline">Edit</a>
                                        <?php endif; ?>
