...
$id_anggota_filter = isset($_GET['id_anggota_ramdan']) ? mysqli_real_escape_string($koneksi, $_GET['id_anggota_ramdan']) : '';
$jenis_filter = isset($_GET['jenis_simpanan_ramdan']) ? mysqli_real_escape_string($koneksi, $_GET['jenis_simpanan_ramdan']) : '';
...
if ($role_user == 'Anggota') {
    $id_user_ramdan = $_SESSION['id_user_ramdan'];
    $q_u = mysqli_query($koneksi, "SELECT id_anggota_ramdan FROM tb_user_ramdan WHERE id_user_ramdan = '$id_user_ramdan'");
    $d_u = mysqli_fetch_assoc($q_u);
    $id_anggota_filter = $d_u['id_anggota_ramdan'] ?? '';
}

// 2. Bangun Query SQL Dinamis
$sql = "SELECT s.*, a.nama_ramdan FROM tb_simpanan_ramdan s 
        JOIN tb_anggota_ramdan a ON s.id_anggota_ramdan = a.id_anggota_ramdan 
        WHERE 1=1";

if ($id_anggota_filter != '') $sql .= " AND s.id_anggota_ramdan = '$id_anggota_filter'";
if ($jenis_filter != '') $sql .= " AND s.jenis_simpanan_ramdan = '$jenis_filter'";
if ($metode_filter != '') $sql .= " AND s.metode_pembayaran_ramdan = '$metode_filter'";

// Filter Range Tanggal
if ($tgl_awal != '' && $tgl_akhir != '') {
    $sql .= " AND s.tanggal_ramdan BETWEEN '$tgl_awal' AND '$tgl_akhir'";
} elseif ($tgl_awal != '') {
    $sql .= " AND s.tanggal_ramdan >= '$tgl_awal'";
} elseif ($tgl_akhir != '') {
    $sql .= " AND s.tanggal_ramdan <= '$tgl_akhir'";
}

$sql .= " ORDER BY s.tanggal_ramdan DESC, s.id_simpanan_ramdan DESC";
...
while($st = mysqli_fetch_assoc($q_stat)){
    if($st['jumlah_ramdan'] > 0) $total_masuk += $st['jumlah_ramdan'];
    else $total_keluar += abs($st['jumlah_ramdan']);
}
$saldo_akhir = $total_masuk - $total_keluar;

// Params untuk link cetak agar hasil PDF sama dengan di layar
$params_cetak = http_build_query([
    'id_anggota_ramdan' => $id_anggota_filter,
    'jenis_simpanan_ramdan' => $jenis_filter,
...
<div class="content">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="cetak_simpanan_ramdan.php?<?php echo $params_cetak; ?>" target="_blank" class="btn btn-dark fw-bold rounded-pill px-4 shadow-sm">
        ...
        <form method="GET">
            <div class="f-container">
                ...
                <div class="f-item" style="min-width: 200px;">
                    ...
                    <select name="id_anggota_ramdan" class="f-control">
                        ...
                        while($d_opt = mysqli_fetch_assoc($q_opt)) {
                            $sel = ($id_anggota_filter == $d_opt['id_anggota_ramdan']) ? 'selected' : '';
                            echo "<option value='".$d_opt['id_anggota_ramdan']."' $sel>".strtoupper($d_opt['nama_ramdan'])."</option>";
                        }
                        ?>
                    </select>
                </div>
                ...
                <div class="f-item">
                    ...
                    <select name="jenis_simpanan_ramdan" class="f-control">
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="riwayat_simpanan_ramdan.php" class="btn-f btn-f-light d-flex align-items-center" title="Reset Filter">
            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    while ($row = mysqli_fetch_assoc($query)): 
                        $is_plus = $row['jumlah_ramdan'] > 0;
                    ?>
                    <tr>
                        <td class="text-center text-muted small"><?php echo $no++; ?></td>
                        <td class="text-center small"><?php echo date('d/m/Y', strtotime($row['tanggal_ramdan'])); ?></td>
                        <td class="fw-bold text-dark"><?php echo strtoupper($row['nama_ramdan']); ?></td>
                        <td>
                            <span class="badge <?php echo $is_plus ? 'bg-success-subtle text-success' : 'bg-danger-subtle text-danger'; ?> rounded-pill border px-3">
                                <?php echo $is_plus ? 'SETORAN '.$row['jenis_simpanan_ramdan'] : 'PENARIKAN TUNAI'; ?>
                            </span>
                        </td>
                        <td class="text-center">
                            <?php if($row['metode_pembayaran_ramdan'] == 'Transfer'): ?>
                        ...
                        <td class="text-end px-4 fw-bold <?php echo $is_plus ? 'text-success' : 'text-danger'; ?>">
                            <?php echo ($is_plus ? '+' : '-') . ' ' . rupiah(abs($row['jumlah_ramdan'])); ?>
                        </td>
                        <td class="text-center">
                            <a href="cetak_struk_simpanan_ramdan.php?id=<?php echo $row['id_simpanan_ramdan']; ?>" target="_blank" class="btn btn-sm btn-light border" title="Cetak Struk">
