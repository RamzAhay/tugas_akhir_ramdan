...
if (isset($_POST['id_pinjaman_ramdan'])) {
    $id_pinjaman_ramdan = $_POST['id_pinjaman_ramdan'];

    // 1. Ambil data pinjaman berdasarkan ID
    $query_pinjaman = mysqli_query($koneksi, "SELECT jumlah_pinjaman_ramdan FROM tb_pinjaman_ramdan WHERE id_pinjaman_ramdan = '$id_pinjaman_ramdan'");
    $data_pinjaman = mysqli_fetch_assoc($query_pinjaman);
    
    $pokok_pinjaman = $data_pinjaman['jumlah_pinjaman_ramdan'];

    // 2. Hitung Bunga (Misalnya 5%)
    $bunga_ramdan = 5 / 100; // 5 Persen
    $total_bunga = $pokok_pinjaman * $bunga_ramdan;

    // 3. Hitung Total Hutang (Pokok + Bunga)
    $total_hutang = $pokok_pinjaman + $total_bunga; // 500.000 + 25.000 = 525.000

    // 4. Ambil total jumlah_ramdan yang sudah dibayar (diangsur) dari tabel angsuran
    $query_angsuran = mysqli_query($koneksi, "SELECT SUM(jumlah_bayar_ramdan) AS total_dibayar FROM tb_angsuran_ramdan WHERE id_pinjaman_ramdan = '$id_pinjaman_ramdan'");
    ...
    $response = array(
        'total_pinjaman_ramdan' => rupiah($total_hutang),
        'sudah_dibayar'  => rupiah($sudah_dibayar),
        'sisa_pinjaman_ramdan'  => rupiah($sisa_hutang)
    );
