...
$q_pinjam = mysqli_query($koneksi, "SELECT p.*, a.nama_ramdan FROM tb_pinjaman_ramdan p 
                                    JOIN tb_anggota_ramdan a ON p.id_anggota_ramdan = a.id_anggota_ramdan 
                                    WHERE p.id_pinjaman_ramdan = '$id_p'");
...
$total_hutang = $d['total_pinjaman_ramdan'];
$sisa_hutang = $d['sisa_pinjaman_ramdan'];
...
$pdf->Cell(5, 6, ':', 0, 0);
$pdf->SetFont('Arial', '', 10);
$pdf->Cell(80, 6, strtoupper($d['nama_ramdan']), 0, 0);
...
if($d['status_pinjaman_ramdan'] == 'Lunas') {
    $pdf->SetTextColor(0, 150, 0);
} elseif ($d['status_pinjaman_ramdan'] == 'Ditolak') {
    $pdf->SetTextColor(255, 0, 0);
}
$pdf->Cell(30, 6, strtoupper($d['status_pinjaman_ramdan']), 0, 1);
...
$pdf->Cell(5, 6, ':', 0, 0);
$pdf->SetFont('Arial', '', 10);
$pdf->Cell(80, 6, date('d M Y', strtotime($d['tanggal_pinjaman_ramdan'])), 0, 0);

$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(35, 6, 'Tenor', 0, 0);
$pdf->Cell(5, 6, ':', 0, 0);
$pdf->SetFont('Arial', '', 10);
$pdf->Cell(30, 6, $d['lama_pinjaman_ramdan'] . ' Bulan', 0, 1);
$pdf->Ln(5);
...
$q_list = mysqli_query($koneksi, "SELECT * FROM tb_angsuran_ramdan WHERE id_pinjaman_ramdan = '$id_p' ORDER BY tanggal_bayar_ramdan ASC, id_angsuran_ramdan ASC");

if (mysqli_num_rows($q_list) > 0) {
    $no = 1;
    while ($r = mysqli_fetch_assoc($q_list)) {
        // Logika default jika kolom metode kosong
        $metode = (isset($r['metode_pembayaran_ramdan']) && $r['metode_pembayaran_ramdan'] != '') ? $r['metode_pembayaran_ramdan'] : 'Tunai';

        $pdf->Cell(10, 8, $no, 1, 0, 'C');
        $pdf->Cell(35, 8, date('d/m/Y', strtotime($r['tanggal_bayar_ramdan'])), 1, 0, 'C');
        $pdf->Cell(55, 8, ' Angsuran Ke-' . $no, 1, 0, 'L');
        $pdf->Cell(45, 8, $metode, 1, 0, 'C');
        $pdf->Cell(45, 8, 'Rp ' . number_format($r['jumlah_bayar_ramdan'], 0, ',', '.'), 1, 1, 'R');
        $no++;
    }
} else {
...
$pdf->Cell(130);
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(60, 5, strtoupper($_SESSION['nama_ramdan']), 0, 1, 'C');
$pdf->Cell(130);
$pdf->SetFont('Arial', '', 10);
$pdf->Cell(60, 5, 'NIP. ...........................', 'T', 1, 'C');

// Output PDF
$pdf->Output('I', 'Riwayat_Angsuran_' . str_replace(' ', '_', $d['nama_ramdan']) . '.pdf');
?>