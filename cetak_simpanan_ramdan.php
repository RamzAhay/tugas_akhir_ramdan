...
$jenis  = mysqli_real_escape_string($koneksi, $_GET['jenis_simpanan_ramdan'] ?? '');
...
if (!empty($id_a)) {
    ...
    if ($d_info) $nama_anggota = strtoupper($d_info['nama_ramdan']);
}

// 3. Bangun Query SQL yang Sinkron dengan riwayat_simpanan_ramdan.php
$sql = "SELECT s.*, a.nama_ramdan FROM tb_simpanan_ramdan s 
        INNER JOIN tb_anggota_ramdan a ON s.id_anggota_ramdan = a.id_anggota_ramdan 
        WHERE 1=1";

if (!empty($id_a))   $sql .= " AND s.id_anggota_ramdan = '$id_a'";
if (!empty($jenis))  $sql .= " AND s.jenis_simpanan_ramdan = '$jenis'";
if (!empty($metode)) $sql .= " AND s.metode_pembayaran_ramdan = '$metode'";

if (!empty($t1) && !empty($t2)) {
    $sql .= " AND s.tanggal_ramdan BETWEEN '$t1' AND '$t2'";
} elseif (!empty($t1)) {
    $sql .= " AND s.tanggal_ramdan >= '$t1'";
} elseif (!empty($t2)) {
    $sql .= " AND s.tanggal_ramdan <= '$t2'";
}

$sql .= " ORDER BY s.tanggal_ramdan ASC, s.id_simpanan_ramdan ASC";
...
while ($row = mysqli_fetch_assoc($query)) {
    ...
    $pdf->Cell(25, 8, date('d/m/Y', strtotime($row['tanggal_ramdan'])), 1, 0, 'C');
    $pdf->Cell(45, 8, substr(strtoupper($row['nama_ramdan']), 0, 20), 1, 0, 'L');
    $pdf->Cell(30, 8, $row['metode_pembayaran_ramdan'], 1, 0, 'C');
    
    if ($row['jumlah_ramdan'] > 0) {
        $pdf->Cell(40, 8, 'Rp ' . number_format($row['jumlah_ramdan'], 0, ',', '.'), 1, 0, 'R');
        $pdf->Cell(40, 8, '-', 1, 1, 'C');
        $total_debit += $row['jumlah_ramdan'];
    } else {
        $pdf->Cell(40, 8, '-', 1, 0, 'C');
        $nominal_tarik = abs($row['jumlah_ramdan']);
