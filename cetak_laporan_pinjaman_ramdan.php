...
// 3. Bangun Kueri SQL (Harus Sesuai dengan data_pinjaman_ramdan.php)
...
if (!empty($status_f)) {
    $sql .= " AND p.status_pinjaman_ramdan = '$status_f'";
}

// Filter Tanggal
if (!empty($tgl1) && !empty($tgl2)) {
    $sql .= " AND p.tanggal_pinjaman_ramdan BETWEEN '$tgl1' AND '$tgl2'";
} elseif (!empty($tgl1)) {
    $sql .= " AND p.tanggal_pinjaman_ramdan >= '$tgl1'";
} elseif (!empty($tgl2)) {
    $sql .= " AND p.tanggal_pinjaman_ramdan <= '$tgl2'";
}
...
if (mysqli_num_rows($query) > 0) {
    while ($row = mysqli_fetch_assoc($query)) {
        ...
        $pdf->Cell(65, 8, ' ' . strtoupper($row['nama_ramdan']), 1, 0, 'L');
        $pdf->Cell(35, 8, date('d/m/Y', strtotime($row['tanggal_pinjaman_ramdan'])), 1, 0, 'C');
        $pdf->Cell(45, 8, rupiah($row['jumlah_pinjaman_ramdan']), 1, 0, 'R');
        $pdf->Cell(45, 8, rupiah($row['sisa_pinjaman_ramdan']), 1, 0, 'R');
        $pdf->Cell(35, 8, $row['lama_pinjaman_ramdan'] . ' Bulan', 1, 0, 'C');
        $pdf->Cell(45, 8, $row['status_pinjaman_ramdan'], 1, 1, 'C');
        
        $total_piutang += $row['sisa_pinjaman_ramdan'];
    }
