...
$q_setor = mysqli_query($koneksi, "SELECT SUM(jumlah_ramdan) as total FROM tb_simpanan_ramdan WHERE tanggal_ramdan = '$hari_ini' AND jumlah_ramdan > 0");
$d_setor = mysqli_fetch_assoc($q_setor);
$setoran_hari_ini = $d_setor['total'] ?? 0;

// 2. Total Penarikan Hari Ini
$q_tarik = mysqli_query($koneksi, "SELECT SUM(jumlah_ramdan) as total FROM tb_simpanan_ramdan WHERE tanggal_ramdan = '$hari_ini' AND jumlah_ramdan < 0");
...
$q_pending = mysqli_query($koneksi, "SELECT COUNT(*) as jml FROM tb_pinjaman_ramdan WHERE status_pinjaman_ramdan = 'Diajukan'");
...
<div class="content">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted mb-0">Selamat bekerja, <strong><?php echo $_SESSION['nama_ramdan']; ?></strong>. Pantau aktivitas koperasi hari ini.</p>
        </div>
        <div class="text-end">
        ...
        <div class="col-lg-4">
            <div class="card border-0 shadow-sm rounded-4 p-3 h-100">
                ...
                <div class="d-flex flex-column gap-2">
                    <a href="tambah_simpanan_ramdan.php" class="quick-link">
                        <i class="bi bi-plus-circle text-success"></i> Input Setoran Baru
                    </a>
                    <a href="tarik_simpanan_ramdan.php" class="quick-link">
                        <i class="bi bi-dash-circle text-danger"></i> Proses Tarik Tunai
                    </a>
                    <a href="tambah_angsuran_ramdan.php" class="quick-link">
                        <i class="bi bi-receipt text-primary"></i> Input Bayar Angsuran
                    </a>
                    <a href="tambah_pinjaman_ramdan.php" class="quick-link">
            ...
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            <?php
                            $q_recent = mysqli_query($koneksi, "SELECT s.*, a.nama_ramdan 
                                                                FROM tb_simpanan_ramdan s 
                                                                JOIN tb_anggota_ramdan a ON s.id_anggota_ramdan = a.id_anggota_ramdan 
                                                                WHERE s.tanggal_ramdan = '$hari_ini' 
                                                                ORDER BY s.id_simpanan_ramdan DESC LIMIT 5");
                            ...
                            while($r = mysqli_fetch_assoc($q_recent)) {
                                $is_plus = $r['jumlah_ramdan'] > 0;
                            ?>
                            <tr>
                                <td class="ps-4">
                                    <div class="fw-bold"><?php echo strtoupper($r['nama_ramdan']); ?></div>
                                    <div class="text-muted small">ID: <?php echo $r['id_anggota_ramdan']; ?></div>
                                </td>
                                <td>
                                ...
                                <td class="text-end pe-4 fw-bold <?php echo $is_plus ? 'text-success' : 'text-danger'; ?>">
                                    <?php echo rupiah(abs($r['jumlah_ramdan'])); ?>
                                </td>
                            </tr>
                            <?php } ?>
                        </tbody>
                ...
                <div class="card-footer bg-white border-0 text-center py-3">
                    <a href="riwayat_simpanan_ramdan.php" class="text-decoration-none small fw-bold text-primary">Lihat Semua Riwayat <i class="bi bi-arrow-right"></i></a>
                </div>
            </div>
        </div>
    </div>
