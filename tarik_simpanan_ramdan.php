...
/**
 * TANGKAP ID DARI URL:
 * Digunakan saat petugas klik tombol "Tarik" di data_simpanan_ramdan.php
 */
$id_anggota_target = isset($_GET['id_anggota_ramdan']) ? trim($_GET['id_anggota_ramdan']) : '';
...
<div class="content">
    <div class="form-container" style="max-width: 800px; margin: 0 auto; background: white; padding: 30px; border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.08);">
        ...
        <form action="proses_tarik_simpanan_ramdan.php" method="POST" id="formTarik">
            <div class="form-group mb-4">
                <label class="label-minimal fw-bold mb-2">Pilih Nama Anggota</label>
                <select name="id_anggota_ramdan" id="id_anggota_ramdan" class="form-select form-select-lg border-2" required>
                    <option value="">-- Cari Nama Anggota --</option>
                    <?php
                    $q_anggota = mysqli_query($koneksi, "SELECT * FROM tb_anggota_ramdan ORDER BY nama_ramdan ASC");
                    while ($d = mysqli_fetch_assoc($q_anggota)) {
                        $selected = ((string)$d['id_anggota_ramdan'] === (string)$id_anggota_target) ? 'selected' : '';
                        echo "<option value='".$d['id_anggota_ramdan']."' $selected>".strtoupper($d['nama_ramdan'])."</option>";
                    }
                    ?>
                </select>
            </div>
            ...
            <div class="form-group mb-4">
                ...
                <input type="date" name="tanggal_ramdan" class="form-control form-control-lg border-2" value="<?php echo date('Y-m-d'); ?>" required>
            </div>
            ...
            <div class="d-grid gap-2 mt-5">
                ...
                <a href="data_simpanan_ramdan.php" class="btn btn-link text-muted text-decoration-none">Batal dan Kembali</a>
            </div>
        </form>
    </div>
</div>
...
$(document).ready(function() {
    function fetchSaldo(id) {
        if (id !== "") {
            $.ajax({
                url: 'get_total_simpanan_ramdan.php',
                type: 'POST',
                data: { id_anggota_ramdan: id },
    ...
    var currentId = $('#id_anggota_ramdan').val();
    if (currentId) fetchSaldo(currentId);

    // Trigger saat ganti anggota
    $('#id_anggota_ramdan').on('change', function() {
        fetchSaldo($(this).val());
