...
$id_angsuran_ramdan = mysqli_real_escape_string($koneksi, $_GET['id']);

/**
 * QUERY UNTUK STRUK:
 * Mengambil detail angsuran, sisa pinjaman terbaru, dan nama_ramdan anggota.
 */
$sql = "SELECT a.*, p.sisa_pinjaman_ramdan, p.id_pinjaman_ramdan, ang.nama_ramdan 
        FROM tb_angsuran_ramdan a 
        JOIN tb_pinjaman_ramdan p ON a.id_pinjaman_ramdan = p.id_pinjaman_ramdan 
        JOIN tb_anggota_ramdan ang ON p.id_anggota_ramdan = ang.id_anggota_ramdan 
        WHERE a.id_angsuran_ramdan = '$id_angsuran_ramdan'";
 ...
 * Menghitung jumlah_ramdan record angsuran untuk pinjaman ini yang ID-nya <= ID saat ini.
 */
$id_p = $data['id_pinjaman_ramdan'];
$q_hitung = mysqli_query($koneksi, "SELECT COUNT(*) as ke FROM tb_angsuran_ramdan WHERE id_pinjaman_ramdan = '$id_p' AND id_angsuran_ramdan <= '$id_angsuran_ramdan'");
$d_hitung = mysqli_fetch_assoc($q_hitung);
$angsuran_ke = $d_hitung['ke'];

// Persiapan Data
$no_struk  = "STR-" . $data['id_angsuran_ramdan'];
$tgl_bayar = date('d/m/Y H:i', strtotime($data['tanggal_bayar_ramdan']));
$nama_ramdan      = strtoupper($data['nama_ramdan']);
$nominal   = "Rp " . number_format($data['jumlah_bayar_ramdan'], 0, ',', '.');
$sisa      = "Rp " . number_format($data['sisa_pinjaman_ramdan'], 0, ',', '.');
