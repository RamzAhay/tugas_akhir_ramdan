...
$q_simpanan = mysqli_query($koneksi, "SELECT SUM(jumlah_ramdan) as total FROM tb_simpanan_ramdan");
$d_simpanan = mysqli_fetch_assoc($q_simpanan);
$total_simpanan = $d_simpanan['total'] ?? 0;

$q_diluar = mysqli_query($koneksi, "SELECT SUM(sisa_pinjaman_ramdan) as total FROM tb_pinjaman_ramdan WHERE status_pinjaman_ramdan = 'Disetujui'");
$d_diluar = mysqli_fetch_assoc($q_diluar);
$uang_diluar = $d_diluar['total'] ?? 0;

$q_lunas = mysqli_query($koneksi, "SELECT SUM(total_pinjaman_ramdan) as total FROM tb_pinjaman_ramdan WHERE status_pinjaman_ramdan = 'Lunas'");
...
<div class="content">
    ...
    <div class="mb-5">
        ...
        <p class="text-muted mb-0">Laporan posisi keuangan KSP RAMDAN per tanggal_ramdan <?php echo date('d M Y'); ?></p>
    </div>
    ...
    <div class="row g-4">
        <!-- Dana Simpanan -->
        <div class="col-md-4">
            <a href="data_simpanan_ramdan.php" class="dash-card shadow-sm p-4">
        ...
        <div class="col-md-4">
            <a href="data_pinjaman_ramdan.php" class="dash-card shadow-sm p-4 border-primary">
                <div class="d-flex align-items-center justify-content-between mb-3">
        ...
        <!-- Total Anggota -->
        <div class="col-md-4">
            <a href="data_anggota_ramdan.php" class="dash-card shadow-sm p-4">
                <div class="d-flex align-items-center justify-content-between mb-3">
        ...
        <div class="col-lg-4">
            <div class="dash-card shadow-sm h-100 p-4" style="cursor: default;">
                ...
                <div class="d-grid mt-4 gap-2">
                    <a href="riwayat_simpanan_ramdan.php" class="btn btn-light border text-dark fw-bold btn-sm py-2 rounded-3">Buka Log Simpanan</a>
                    <a href="riwayat_pinjaman_ramdan.php" class="btn btn-light border text-dark fw-bold btn-sm py-2 rounded-3">Buka Riwayat Pinjaman</a>
                </div>
            </div>
        </div>
    </div>
