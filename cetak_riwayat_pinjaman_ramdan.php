...
$id_a = mysqli_real_escape_string($koneksi, $_GET['id_anggota_ramdan'] ?? '');
...
if (!empty($id_a)) {
    $q_info = mysqli_query($koneksi, "SELECT nama_ramdan FROM tb_anggota_ramdan WHERE id_anggota_ramdan = '$id_a'");
    $d_info = mysqli_fetch_assoc($q_info);
    if ($d_info) {
        $nama_filter = strtoupper($d_info['nama_ramdan']);
    }
}
...
// Filter Nama Anggota
if (!empty($id_a)) {
    $sql .= " AND p.id_anggota_ramdan = '$id_a'";
}
...
if (mysqli_num_rows($query) > 0) {
    while ($row = mysqli_fetch_assoc($query)) {
        ...
        $pdf->Cell(55, 8, ' ' . strtoupper($row['nama_ramdan']), 1, 0, 'L');
        $pdf->Cell(30, 8, date('d/m/Y', strtotime($row['tanggal_pinjaman_ramdan'])), 1, 0, 'C');
        
        $tgl_lunas = ($row['status_pinjaman_ramdan'] == 'Lunas' && $row['tanggal_lunas']) ? date('d/m/Y', strtotime($row['tanggal_lunas'])) : '-';
        $pdf->Cell(30, 8, $tgl_lunas, 1, 0, 'C');
        
        $pdf->Cell(40, 8, rupiah($row['jumlah_pinjaman_ramdan']), 1, 0, 'R');
        $pdf->Cell(40, 8, rupiah($row['total_pinjaman_ramdan']), 1, 0, 'R');
        $pdf->Cell(35, 8, $row['lama_pinjaman_ramdan'] . ' Bulan', 1, 0, 'C');
        
        if($row['status_pinjaman_ramdan'] == 'Ditolak') $pdf->SetTextColor(200, 0, 0);
        elseif($row['status_pinjaman_ramdan'] == 'Lunas') $pdf->SetTextColor(0, 150, 0);
        
        $pdf->Cell(40, 8, $row['status_pinjaman_ramdan'], 1, 1, 'C');
        $pdf->SetTextColor(0, 0, 0);
    }
} else {
...
$pdf->Cell(200);
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(80, 5, strtoupper($_SESSION['nama_ramdan']), 0, 1, 'C');
$pdf->Cell(200);
$pdf->SetFont('Arial', '', 8);
