...
<div class="content">
    ...
    <div class="f-section shadow-sm">
        <form method="GET" action="riwayat_pinjaman_ramdan.php">
            <div class="f-container">
                ...
                <div class="btn-action-group">
                    ...
                    <a href="cetak_riwayat_pinjaman_ramdan.php?<?php echo $params_cetak; ?>" 
                       class="btn-f-print" target="_blank" title="Cetak Laporan">
                        <i class="bi bi-printer me-2"></i> CETAK
                    </a>

                    <?php if($id_anggota_filter != '' || $tgl_awal != '' || $tgl_akhir != ''): ?>
                        <a href="riwayat_pinjaman_ramdan.php" class="btn-f-reset" title="Bersihkan Filter">
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        $sql = "SELECT p.*, a.nama_ramdan, 
                                (SELECT MAX(tanggal_bayar_ramdan) FROM tb_angsuran_ramdan WHERE id_pinjaman_ramdan = p.id_pinjaman_ramdan) as tanggal_lunas
                                FROM tb_pinjaman_ramdan p 
                                JOIN tb_anggota_ramdan a ON p.id_anggota_ramdan = a.id_anggota_ramdan 
                                WHERE p.status_pinjaman_ramdan != 'Diajukan'";

                        // Filter Anggota
                        if ($id_anggota_filter != '') $sql .= " AND p.id_anggota_ramdan = '$id_anggota_filter'";
                        
                        // Logika Filter Tanggal Berdasarkan Tipe (Pinjam / Lunas)
                        $kolom_tgl = ($filter_tipe == 'lunas') ? "(SELECT MAX(tanggal_bayar_ramdan) FROM tb_angsuran_ramdan WHERE id_pinjaman_ramdan = p.id_pinjaman_ramdan)" : "p.tanggal_pinjaman_ramdan";
                         ...
                         * Disamakan dengan file cetak_riwayat_pinjaman_ramdan.php
                         */
                        $sql .= " ORDER BY p.tanggal_pinjaman_ramdan DESC";
                        ...
                        while ($data = mysqli_fetch_assoc($query)) {
                            ...
                            // Logika tampilan tanggal_ramdan lunas
                        ...
                        <tr>
                            ...
                            <td class="fw-bold"><?php echo htmlspecialchars($data['nama_ramdan']); ?></td>
                            ...
                            <td><?php echo rupiah($data['jumlah_pinjaman_ramdan']); ?></td>
                            <td class="fw-bold"><?php echo rupiah($data['total_pinjaman_ramdan']); ?></td>
                            <td class="text-center">
                                <span class="badge <?php echo $badge; ?> rounded-pill px-3"><?php echo $status; ?></span>
                            </td>
                            <td class="text-center">
                                <?php if($status != 'Ditolak'): ?>
                                    <a href="riwayat_angsuran_ramdan.php?id=<?php echo $data['id_pinjaman_ramdan']; ?>" class="btn btn-outline-info btn-sm py-1 px-2" style="font-size: 11px;">Detail</a>
                                <?php else: ?>
