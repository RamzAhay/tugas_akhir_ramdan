    ...
    <div class="row justify-content-center">
        <div class="col-md-7 col-lg-6">
            <div class="form-angsuran-card p-4 p-md-5">
                ...
                <form action="proses_tambah_angsuran_ramdan.php" method="POST">
                    
                    <!-- Pilih Anggota & Pinjaman -->
                    <div class="mb-4">
                        <label class="label-minimal d-block mb-2">Anggota dengan Pinjaman Aktif</label>
                        <select name="id_pinjaman_ramdan" id="id_pinjaman_ramdan" class="form-select input-clean" required onchange="updateDebtInfo()">
                            <option value="">-- Cari Nama Anggota --</option>
                            <?php
                            /**
                             * PERUBAHAN: Order by id_anggota_ramdan ASC (melalui tabel p)
                             * TAMPILAN: Hanya menampilkan nama_ramdan
                             */
                            $query = mysqli_query($koneksi, "SELECT p.*, a.nama_ramdan 
                                                            FROM tb_pinjaman_ramdan p 
                                                            JOIN tb_anggota_ramdan a ON p.id_anggota_ramdan = a.id_anggota_ramdan 
                                                            WHERE p.status_pinjaman_ramdan = 'Disetujui' 
                                                            ORDER BY a.id_anggota_ramdan ASC");
                            
                            while ($row = mysqli_fetch_assoc($query)) {
                                $selected = ($id_pinjaman_get == $row['id_pinjaman_ramdan']) ? 'selected' : '';
                                
                                $sisa = (int)$row['sisa_pinjaman_ramdan'];
                                $total = (int)$row['total_pinjaman_ramdan'];
                                $bunga_ramdan = $row['bunga_ramdan'];

                                echo "<option value='".$row['id_pinjaman_ramdan']."' 
                                              data-sisa='$sisa' 
                                              data-total='$total' 
                                              data-bunga_ramdan='$bunga_ramdan'
                                              $selected>";
                                echo htmlspecialchars($row['nama_ramdan']);
                                echo "</option>";
                            }
                            ?>
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="hidden" name="jumlah_bayar_ramdan" id="jumlah_bayar_ramdan">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="metode_pembayaran_ramdan" class="form-select input-clean" required>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" name="tanggal_bayar_ramdan" class="form-control input-clean w-100" value="<?php echo date('Y-m-d'); ?>" required>
                    </div>
                    ...
                    <div class="d-grid mt-5">
                        ...
                        <a href="data_angsuran_ramdan.php" class="btn btn-link mt-2 text-decoration-none text-muted small text-center">Kembali ke Daftar</a>
                    </div>
                </form>
            </div>
        </div>
...
<script>
    ...
    function handlePaymentInput(input) {
        ...
        document.getElementById('jumlah_bayar_ramdan').value = rawValue;
    ...
    function updateDebtInfo() {
        const sel = document.getElementById('id_pinjaman_ramdan');
    ...
    function checkPayment() {
        const bayarRaw = document.getElementById('jumlah_bayar_ramdan').value;
